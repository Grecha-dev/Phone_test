...
const defaultSettings = () => ({
    ...
    socialProfileId: '',
    // Свой ключ API для телефона: запросы идут напрямую к провайдеру из профиля,
    // минуя глобальные секреты ST → чат и телефон работают одновременно без гонки.
    // Пусто — классический путь через профиль (с ротацией активного секрета).
    directApiKey: '',
